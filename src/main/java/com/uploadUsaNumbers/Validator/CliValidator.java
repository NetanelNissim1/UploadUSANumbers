...
public class CliValidator {
    ...
    public static ValidCliWithCountryCode getValidE164CellularPhoneNumberAndCC(String cli,
            final Phonenumber.PhoneNumber buffer, boolean exactMatch) {
        return getValidE164CellularPhoneNumberAndCC(cli, null, buffer, exactMatch);
    }

    public static ValidCliWithCountryCode getValidE164CellularPhoneNumberAndCC(String cli, String defaultRegion,
            final Phonenumber.PhoneNumber buffer, boolean exactMatch) {
        ...
        try {
            ...
            if (PHONE_UTIL.isValidNumber(workingBuffer)) {
                ...
                if (/* type == PhoneNumberUtil.PhoneNumberType.TOLL_FREE || */ type == PhoneNumberUtil.PhoneNumberType.MOBILE
                        || type == PhoneNumberUtil.PhoneNumberType.FIXED_LINE_OR_MOBILE
                        || (type == PhoneNumberUtil.PhoneNumberType.FIXED_LINE && "MX".equals(cc))) {
                    String phone = subtractLeadingPlus(
                            PHONE_UTIL.format(workingBuffer, PhoneNumberUtil.PhoneNumberFormat.E164));
    ...
    public static String subtractLeadingPlus(String possiblyCliWithLeadingPlus) {
        if (possiblyCliWithLeadingPlus != null && possiblyCliWithLeadingPlus.isEmpty() == false
    ...
    public static ValidCliWithCountryCode getValidE164PhoneNumberAndCC(String possiblyCli,
            final Phonenumber.PhoneNumber buffer) {
        Phonenumber.PhoneNumber workingBuffer;
        String resultCli = null;
        String cc = null;
        if (buffer == null) {
